import { buyStonks, sellStonks, addToCart, store, Stonk, State } from './exports'
...
// creates request to update state
const createPostData = (state: State): PostData => ({
  cart: state.cart,
  inventory: state.inventory,
  money: state.money
})
export function* getState() {
  yield takeEvery('GET_STATE_ASYNC', () => {
    fetch('/state').then(res => {
      store.dispatch({ type: 'SET_STATE_TEST', newState: res })
...
export function* postStonkToCart() {
  ...
  yield takeEvery('POST_STONK_TO_CART', (action: MyAction) => { // gets called on actions with type of 'POST_STONK_TO_CART'
    ...
    return axios.post('/stonk/add-to-cart', createPostData(store.getState())).catch((err: any) => console.log(err))
  })
}
